...
typedef enum EFUN_ATTR
{
    ...
    EFUN_ATTR_LOGIN_SUP_RSA_ENC = 16, // 登录加密是否支持RSA，0:不支持RSA  1:支持RSA(默认)（ 为了临时解决设备端对登录RSA加密校验方式和sdk这边的方式不同）
...
//返回设备是否开启了微信报警推送 ？？这这个代码有缺陷，只是从登陆时缓存信息获取（微信报警状态会改变的，打开/关闭），使用 FUN_SysWXAlarmStateCheck每次去服务器查询
...
/*******************用户服务相关的接口**************************
 * 方法名: 云平台app证书合法性校验-不合法UUID或者AppKey不允许使用
 * 描  述:云平台app证书合法性校验-不合法UUID或者AppKey不允许使用;
 *       在SDK初始化之后，后台异步从服务器校验SDK合法性，如果是不合法的情况下，设备登录和视频预览等关键功能不允许使用；
 *       默认或者服务器无法访问的情况下，允许使用SDK，不做限制.
 * 返回值:
 *      [无]
 * 参  数:
 *      输入(in)
 ...
 *          [无]
 * 结果消息：
 *         消息EMSG_SYS_FACE_CHECK_OCX 5080
 *         arg1: >=0 成功，< 0错误值，详见错误码
 *         Str: json返回的data信息
 ****************************************************/
int FUN_SysFaceCheckOcx(UI_HANDLE hUser, int nSeq = 0);
 ...
 *         arg1: >=0 成功，<0 错误值，详见错误码
 *         str：返回szType参数
 *         pData：类型是fp的时候返回用户名，其他时候不返回
 ****************************************************/
int FUN_SysSendGlobalPhoneCode(UI_HANDLE hUser, const char *szPhone, const char *szType, const char *szAreaCode, int nSeq = 0);
...
// 是否开通云存储 "XXXAbillity/CloudStore": -1：未知；0（不支持）、 1（支持已开通，正常使用）、2（支持已开通，服务到期） 、3（支持未开通） *不会再通过EKEY_VALUE_DEVICES_CS获取，所以此能力级systemfunction里面没有的话，无法获取
...
*          [无]
* 结果消息：
* 消息ID：EMSG_START_PLAY = 5501：start 结果返回 /  EMSG_DEV__RETURN REAL_STREAM_START：媒体数据开始实时返回，dss密码错误返回（param1只有dss错误才会返回其他都是0）  / EMSG_ON_MEDIA_DATA = 5533：数据返回
* EMSG_ON_MEDIA_DATA回调参数说明：param1：数据长度； param2：码流类型； param3：码流子类型 ；pData：媒体数据，包含header frame
*码流类型：
